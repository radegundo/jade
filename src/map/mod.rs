...
pub fn solid_wall(x0: f32, y0: f32, x1: f32, y1: f32, color: Color) -> LineDef {
    LineDef::new(x0, y0, x1, y1, color)
}
...
/// All four walls are solid and single-colored. Useful for quick test rooms.
pub fn rect_sector(
    self_index: usize,
    min: Vec2,
    max: Vec2,
    ...
    SectorBuilder::new(self_index, floor_height, ceiling_height)
        ...
        .wall(min.x, max.y, min.x, min.y, wall_color)
        .build()
}
